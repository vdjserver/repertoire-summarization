...
taken as input and several files are generated as outputs:
...
5) JSON-formatted sample (file) - level characterization data.

The rep_char.py script is designed to support the above functions.
...
For the .sh scripts please open them and read the comments and examine 
...
usage: rep_char.py [-h] [-igblast_version IGBLAST_VERSION]
                   ...
                   [-json_out JSON_OUT] [-combo_out COMBO_OUT]
                   [-cdr3_hist_out CDR3_HIST_OUT]
                   [-sample_json_out SAMPLE_JSON_OUT] [-skip_char]
                   igblast_in vdjml_out char_out vdj_db_root qry_fasta
                   {human,Mus_musculus}

Parse IGBLAST output, write a VDJML output file. Generate read-level
repertoire-characterization data. VERSION vDIOGENIX_1_USED_TAG-16-g480fdf3
...
optional arguments:
  ...
  -combo_out COMBO_OUT  output file for JSON segment recombination count data
  -cdr3_hist_out CDR3_HIST_OUT
                        output file for the CDR3 histogram of lengths (both
                        kabat and imgt systems)
  -sample_json_out SAMPLE_JSON_OUT
                        output file for sample-level JSON repertoire data
                        (e.g. R:S over IMGT CDRs)
  -skip_char            If this is set, then characterization besides germline
                        segment assignment and CDR3 analysis is skipped
...
Tse of rep_char.py require that the python module "vdjml" be usable. Moreover
version 0.0.12 is required.  To be usable, the module must be loaded by PYTHON

NOTE : rep_char.py expects to be able to find the file "vdjml_igblast_parse.py". 
It is part of the the VJDML package. Successful finding may be 
achieved by using the PYTHONPATH. Adding the directory containing
"vdjml_igblast_parse.py" can help to address the issue.
...
files and directories downloaded/created and expected to be in a
specific organized manner.
...
Under the Test_Data directory lies a driver 
script and input data to test some core 
functionalities.

The driver script is tests.sh.  Besides testing, 
it can be viewed to see some example usage.
...
in both IMGT and KABAT domain modes (except for TR which is
only analyzed in IMGT mode) .  Also, during the course of
...
(TSV-formatted), CDR3 length histograms, and JSON data.
Output/error log files are also generated.
