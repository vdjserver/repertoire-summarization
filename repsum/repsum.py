...
def summary():
	...
	if (not args):
        ...
        while vr.has_result():
                ...
                read_analysis_results=summarize.rep_char_read(read_result_obj,meta,organism,imgtdb_obj,query_record,args.skip_char)
...
def main():
	...
	print(inputDict[defaults.metadata_file_key])
        try:
                infile = open(inputDict[defaults.metadata_file_key])
        ...
        # Files
        try:
                infile = open(inputDict[defaults.files_key])
                filesDict = json.load(infile)
        except:
                print("Could not read input specification file: " + input_file)
                raise
        else:
